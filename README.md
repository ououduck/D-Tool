...
- **在线 API 工具**：接入免费公共 API（IP 归属、全球天气、实时汇率、一言、笑话、随机用户、密码泄露检查等），无需注册密钥，页面带加载态与错误提示
- **3D 交互体验**：掷骰子/抛硬币使用 CSS 3D 动画真实滚动，猜拳可视化对战带计分与历史
...
| 网络信息 | 33 | IP 查询、子网计算、IP 进制、UA 解析、HTTP 头/报文、Cookie、URL 参数、编码检测、端口查询；在线 API：我的 IP/全球天气/实时汇率/一言/英文笑话/随机用户/姓名预测/密码泄露检查/随机狗图/国家信息/二次元壁纸（竖版横版）/风景图/原神图 |
| 生活实用 | 42 | BMI、体脂率、腰臀比、热量、睡眠、预产期、宠物年龄、酒驾估算、彩票、塔罗、姓名/星座/血型配对、情书、藏头诗、笑话、谜语、绕口令；3D 骰子、3D 抛硬币、猜拳对战等 |
